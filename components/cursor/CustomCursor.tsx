...
export function CustomCursor() {
  ...
  useEffect(() => {
    const checkDesktop = () => {
      // Use 1024px for desktop to avoid issues on tablets
    ...
    const coreX = gsap.quickTo(core.current, "x", { duration: 0.1, ease: "power3" })
    const coreY = gsap.quickTo(core.current, "y", { duration: 0.1, ease: "power3" })
    const auraX = gsap.quickTo(aura.current, "x", { duration: 0.5, ease: "power4.out" })
    const auraY = gsap.quickTo(aura.current, "y", { duration: 0.5, ease: "power4.out" })
    ...
    const render = () => {
      ...
      const stretch = 1 + Math.min(speed / 150, 0.4)
      const squash = 1 - Math.min(speed / 200, 0.2)
    ...
    const handleViewEnter = () => {
      ...
      gsap.to(label.current, { opacity: 1, scale: 1, duration: 0.4, delay: 0.1 })
      gsap.to(core.current, { opacity: 0, scale: 0, duration: 0.2 })
    }

    const handleViewLeave = () => {
      gsap.to(label.current, { opacity: 0, scale: 0.5, duration: 0.3 })
      handleMouseLeave()
    }
    ...
    // Watch for dynamic DOM changes (e.g., page transitions)
    ...
    const handleClick = () => {
      ...
      gsap.to(aura.current, {
        ...
        duration: 0.1,
        onComplete: () => {
          gsap.to(aura.current, { scale: 1, duration: 0.5, ease: "elastic.out(1, 0.3)" })
        }
      })
    }
    ...
    <>
      ...
      <div 
        ...
        style={{ 
          ...
          transition: "width 0.4s cubic-bezier(0.23, 1, 0.32, 1), height 0.4s cubic-bezier(0.23, 1, 0.32, 1), background-color 0.4s, border-color 0.4s"
        }}
      >
